...
-- ensuite, nous voulons remplir les attributs nom, prenom et adresse avec du texte et entre 30/50 charactères et ils doivent être obligatoirement saisi
-- nous ajouté un champ pxabonnement, qui va correspondre donc au prix de l'abonnement que le membre à prit chez un producteur 
...
-- création de la table producteurs, le numéro du producteur doit commencer par 'PDT' et se terminer par 2 chiffresque nous metterons lors de l'insertion 
-- ensuite, même chose que pour les membres, nous voulons remplir les attributs nom, prenom et adresse avec du texte et entre 30/50 charactères 
...
-- ensuite, nous avons ddécisder de supprimer les attributs Cru et Région, et de les remplacer par les attributs pxvente, chargevariable, chargefixe 
-- Ces nouveaux attributs nous sembe plus intérréssant, car nous pouvons construire un sujet du même type qu'un exercice de Contrôle de Gestion 
...
--créationd de la table commandes, pas de suppression ou d'ajout d'attributs
...
-- Nous avons attribué à l'attribut datecommande un type DATE pour saisir une date et nous avons attribué untype SMALLINT à l'attribut qtecommandee 
-- (donc les qtecommandee sont entre -32 et 32, approximativement)
...
-- Cette requête à pour objectif de renvoyer combien de bouteilles commandées il y a entre février/mai et décembre--
...
-- Cette requête à pour objectif de renvoyer les clients qui sont de la même ville que le producteur Jean--

select * from membres 
WHERE ville IN (select ville from producteurs WHERE producteurs.nom='Jean')
...
GROUP BY noproducteur, novin, nb_commande, pxvente, CA, cv, chargefixe,resultat
